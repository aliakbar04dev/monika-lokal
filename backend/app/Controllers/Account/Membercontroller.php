            ...
            if ($this->request->isAJAX())
            {
                $validationCheck = $this->validate([
                    ...
                    'accountmember_nama' => [
                        ...
                        'rules' => [
                            'required',
                            'is_unique[m_jenis_member.jenis_member]',
                        ],
                        'errors' => [
                            'required' 		=> '{field} wajib terisi',
                            'is_unique'	    => '{field} tidak boleh sama, isikan dengan data lain'
                        ],
                    ]
                ]);
            }
        ...
        else
        {
            if ($this->request->isAJAX())
            {
                ...
                else
                {
                    ...
                    $tmp   = $m_member->checkjenis($kode);
                    $tmpCheck = $tmp[0]['jenis_member'];

                    $jenis = $this->request->getVar('accountmember_namaubah');
                    ...
                    else
                    {
                        $check = $this->validate([
                            'accountmember_namaubah' => [
                                ...
                                'errors' => [
                                    'required' 		=> '{field} wajib terisi',
                                    'is_unique'	    => '{field} tidak boleh sama, masukkan dengan data lain'
                                ],
                            ],
                        ]);
