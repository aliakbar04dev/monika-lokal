	...
	function register()
	{
		...
		try {
			...
			if ($decoded) {
				...
				if ($nohp != '') {
					$usr = $userModel->select('kode_user')->where('no_hp', $nohp)->where('is_verif', 0)->first();
	...
	function resetphoneWithoutOTP()
	{
		...
		try {
			...
			if ($decoded) {
				...
				$nohp = $this->request->getVar('new_phonenumber');
				$pass = md5($this->request->getVar('password'));
				//$otp  = $this->get_random_number();
				//$otp  = $this->generateNumericOTP(6);
				//$pesan = "Kami menerima permintaan perubahan password akun Monika PanenSaham anda. Masukkan kode " . $otp . " untuk melanjutkan proses perubahan password";
				...
				if ($email != '' && $pass != '') {
					$usr = $userModel->select('kode_user')->where('alamat_email', $email)->where('is_verif', 1)->where('password', $pass)->first();
					...
					if (!empty($usr)) {
						if (!is_null($usr) && count($usr) > 0) {
							...
							$response = [
								'status' => 200,
								'error' => FALSE,
								'messages' => 'Phone has update succesfully',
							];
							return $this->respondCreated($response);
						} else {
							$response = [
