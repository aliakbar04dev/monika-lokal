<div class="col-lg-3 col-md-3">
    ...
    <div class="col-12 pb-4 background-color">
        <button class="collapsible radiusatas">
            <i class="fas fa-sliders-h" style="color:#fdc134;"></i><a class="judulmenu bold" style="font-family: 'Poppins'; font-weight:bold; font-size:12px;">STATUS FILTER</a>
        </button>
        <div class="content" style="max-height: 234px;">
            <button onclick="window.location.href='<?= site_url('billing'); ?>'" class="dalem <?= ($title == 'allPaid') ? 'activee' : '' ?>">
                <i class="far fa-circle" style="color:<?= ($title == 'allPaid') ? '#fff;' : '#fdc134;' ?>"></i><a class="judulmenu" style="font-family: 'Poppins'; font-weight:bold; font-size:12px;">Semua Pembayaran</a>
            </button>
            <button onclick="window.location.href='<?= site_url('paid'); ?>'" class="dalem <?= ($title == 'Paid') ? 'activee' : '' ?>">
                <i class="far fa-circle" style="color:<?= ($title == 'Paid') ? '#fff;' : '#fdc134;' ?>"></i><a class="judulmenu" style="font-family: 'Poppins'; font-weight:bold; font-size:12px;">Sudah dibayar</a>
            </button>
            <button onclick="window.location.href='<?= site_url('unpaid'); ?>'" class="dalem <?= ($title == 'Unpaid') ? 'activee' : '' ?>">
                <i class="far fa-circle" style="color:<?= ($title == 'Unpaid') ? '#fff;' : '#fdc134;' ?>"></i><a class="judulmenu" style="font-family: 'Poppins'; font-weight:bold; font-size:12px;">Belum Dibayar</a>
            </button>
            <button onclick="window.location.href='<?= site_url('cancel'); ?>'" class="dalem radiusbawah <?= ($title == 'Cancelled') ? 'activee' : '' ?>">
                <i class="far fa-circle" style="color:<?= ($title == 'Cancelled') ? '#fff;' : '#fdc134;' ?>"></i><a class="judulmenu" style="font-family: 'Poppins'; font-weight:bold; font-size:12px;">Cancel</a>
            </button>
            <!--
