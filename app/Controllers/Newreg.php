...
class Newreg extends BaseController
{
    ...
    public function __construct()
    {
		...
		$this->regisTmpModel = new RegisTmpModel();
    ...
    public function reg(){
        if (!hassession('email')) {
            if(hassession('regis_after_auto')){
                ...
                // $referalRegis = $request->getVar('referal');
                ...
                $data = array(
                    ...
                    'reference_id' => getsession('reference_id'),
                );

                // echo json_encode($data['reference_id']); die;
        ...
        if ($this->request->isAJAX()) {
            if (hassession('email')) {
                ...
                if(!hassession('regis_phone_code_user') && !hassession('regis_phone_input')){
                    ...
                    $this->regisTmpModel->update($kode_user, $data);
        ...
        if ($this->request->isAJAX()) {
            if (hassession('email')) {
                ...
                if(!hassession('regis_otp_input') && !hassession('regis_otp_phone') && !hassession('regis_otp_code_user')){
                    ...
                    $usr		= $this->regisTmpModel->select('regis_no_hp, regis_otp, regis_otp_exp')->where('kode_user', $kode_user)->first();
                    ...
                    if($regisotp != '' && $otpexp != ''){
                        if($otpexp >= $now){
                            if($otpcode == $regisotp){
								...
								$query = $this->regisTmpModel->select('*')->where('kode_user', $kode_user)->first();
								$this->regisModel->insert($query);
                                ...
                                if($viapemb != ''){
                                    ...
                                    $dataUser = $this->regisTmpModel->where('kode_user', $kode_user)
